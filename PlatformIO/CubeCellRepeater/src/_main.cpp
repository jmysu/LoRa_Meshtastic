...
//#define SILENT         // define to SILENT to turn off serial messages
...
void onRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    if ( !(size > sizeof(PacketHeader)) ) {
            ...
            MSG("Packet too small (No MeshPacket).\n");
        #endif
        #ifndef NOBLINK
    ...
    display.drawXbm(display.getWidth()-icon_width, 0, icon_width, icon_height, icon_bits); //Show Meshtastic icon
    display.display();
#endif
